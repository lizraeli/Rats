...
test("should render a single image as a Rat Marker", () => {
...
test("should render a non-selected Rat Marker without the 'selected' className", () => {
  const wrapper = shallow(<RatMarker image={ratImageS} selected={false} />);
  expect(wrapper.hasClass("selected")).toEqual(false);
  expect(shallowToJson(wrapper)).toMatchSnapshot();
});

test("should renders a selected Rat Marker with the 'selected' className", () => {
